...
function absoluteCheckUrl(bodyCheckUrl: string, settings: AnyRecord, reservation: AnyRecord) {
  ...
  const raw =
    bodyCheckUrl ||
    `${String(base).replace(/\/$/, '')}/check?id=${encodeURIComponent(reservation?.id || '')}`;
...
async function qrDataUrl(value: string) {
  try {
    ...
    console.error('EMAIL_QR_ERROR', error);
    return '';
  }
}
...
function reservationText(reservation: AnyRecord, settings: AnyRecord, checkUrl: string) {
  const intro =
    settings?.emailIntro ||
    'Dobrý den, děkujeme za Vaši rezervaci. Níže najdete detail termínu.';

  const footer =
    settings?.emailFooter ||
    'Těšíme se na Vás. Tým Střelnice Karlín';
...
async function reservationHtml(reservation: AnyRecord, settings: AnyRecord, checkUrl: string) {
  const title =
    ...
    'Potvrzení rezervace – Střelnice Karlín';
...
function simplePdfCard(reservation: AnyRecord, settings: AnyRecord, checkUrl: string) {
  const lines = [
    ...
    '',
    ...reservationRows(reservation, settings).map(
      ([k, v]) => `${pdfEscape(k)}: ${pdfEscape(v)}`
    ),
    '',
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    port: cfg.port,
    secure: cfg.secure,
    from: cfg.from ? cfg.from.replace(/<.*>/, '<***>') : null,
  });
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!to || !to.includes('@')) {
      return NextResponse.json(
        { ok: false, message: 'Chybí platný e-mail klienta.' },
        { status: 400 }
      );
    }
...
const qrBase64 = qrData.replace(/^data:image\/png;base64,/, '');
    ...
    if (!cfg.ready) {
      ...
      return NextResponse.json({
        ok: true,
        mode: 'demo',
        message: 'E-mail je připravený. Pro reálné odeslání nastavte SMTP.',
      });
    }
  ...
  settings?.emailAttachCard === false
    ...
    return NextResponse.json({
      ok: true,
      mode: 'smtp',
      message: 'E-mail byl odeslán přes SMTP.',
    });
  } catch (error: any) {
    console.error('EMAIL_SEND_ERROR', error);

    return NextResponse.json(
      { ok: false, message: error?.message || 'E-mail se nepodařilo odeslat.' },
      { status: 500 }
    );
  }
}
